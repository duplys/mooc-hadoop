...
Takes a <word, value> pair as input and joins the words together. The input
comes as a group of lines with the same word as the key.

As it reads words it will hold on to the value field. It will keep track of
current word and previous word, if word changes then it will perform the 
'join' on the set of held values by merely printing out the word and 
values. At the end it will perform the last join
"""
